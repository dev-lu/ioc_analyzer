...
def abuseipdb_ip_check(ip, apikey):
    ...
    if response.status_code == 429:
        ...
        if int(abuseipdb_score)<1:
            table.add_row([
                "AbuseIPDB", 
                 f"{abuseipdb_score}% malicious - {abuseipdb_reports} report(s)", 
                green
            ])
        elif int(abuseipdb_score)<80:
            table.add_row([
                "AbuseIPDB", 
                f"{abuseipdb_score}% malicious - {abuseipdb_reports} report(s)", 
                yellow
            ])
        elif int(abuseipdb_score)>79:
            table.add_row([
                "AbuseIPDB", 
                f"{abuseipdb_score}% malicious - {abuseipdb_reports} report(s)", 
                red
...
def alienvaultotx(ioc, endpoint, apikey):
    ...
    if response.status_code == 200:
        ...
        if int(alienvault_score)<1:
            table.add_row([
                "Alienvault", 
                f"Pulses: {alienvault_score}", 
                green
            ])
        elif int(alienvault_score)>0:
            table.add_row([
                "Alienvault", 
                f"Pulses: {alienvault_score}", 
                red
...
def virustotal(ioc, endpoint, apikey):
    ...
    if response.status_code == 429:
        ...
        if int(virustotal_score)<1:
            table.add_row([
                "Virustotal", 
                f"Identified as malicious by {virustotal_score} Services.", 
                green
            ])
        elif int(virustotal_score)<2:
            table.add_row([
                "Virustotal", 
                f"Identified as malicious by {virustotal_score} Services.", 
                yellow
            ])
        elif int(virustotal_score)>1:
            table.add_row([
                "Virustotal", 
                f"Identified as malicious by {virustotal_score} Services.", 
                red
...
def ipqualityscore_ip_check(ip, apikey):
    ...
    if response_json["success"] and response.status_code == 200:
        if int(ipqualityscore_score)<1:
            table.add_row([
                "IPQualityScore",
                f"{ipqualityscore_score}% malicious", 
                green
            ])
        elif int(ipqualityscore_score)<90:
                table.add_row([
                    "IPQualityScore", 
                    f"{ipqualityscore_score}% malicious",
                    yellow
                ])
        elif int(ipqualityscore_score)>89:
                table.add_row([
                    "IPQualityScore", 
                    f"{ipqualityscore_score}% malicious",
                    red
...
def blocklist_de_ip_check(ip):
    ...
    if response.status_code == 200:
        if int(blocklist_attacks)<1 and int(blocklist_reports)<1:
            table.add_row([
                "Blocklist.de", 
                f"Attacks: {blocklist_attacks} - Reports: {blocklist_reports}", 
                green
            ])
        else:
            table.add_row([
...
def maltiverse_ip_check(ip, apikey):
    ...
    if response.status_code == 429:
        ...
        if "is_distributing_malware" in response_json:
            ...
            if maltiverse_is_distributing_malware == "True":
                table.add_row([
                    ...
                    maltiverse_is_distributing_malware,
                    red
                ])
            else:
                table.add_row([
        ...
        if "is_hosting" in response_json:
            maltiverse_is_hosting = str(response_json["is_hosting"])
        ...
        if "as_name" in response_json:
            print("ASN: " + str(response_json["as_name"]))
        if "asn_cidr" in response_json:
            print("ASN CIDR: " + str(response_json["asn_cidr"]))
        if "asn_country_code" in response_json:
            print("ASN country: " + str(response_json["asn_country_code"]))
        if "asn_date" in response_json:
            print("ASN Date: " + str(response_json["asn_date"]))
        if "asn_registry" in response_json:
            print("ASN registry: " + str(response_json["asn_registry"]))
        if "city" in response_json:
            print("City: " + str(response_json["city"]))
        if "creation_time" in response_json:
            print("Creation time: " + str(response_json["creation_time"]))
        if "modification_time" in response_json:
            print("Last modification: " + str(response_json["modification_time"]))
        if "number_of_domains_resolving" in response_json:
            print("Number of domains resolving: " + str(response_json["number_of_domains_resolving"]))
        if "number_of_blacklisted_domains_resolving" in response_json:
            print("Number of blacklisted domains: " + str(response_json["number_of_blacklisted_domains_resolving"]))
        if "number_of_whitelisted_domains_resolving" in response_json:
            print("Number of whitelisted Domains: " + str(response_json["number_of_whitelisted_domains_resolving"]))
